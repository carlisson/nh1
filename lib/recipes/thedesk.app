...
function APP_VERSION {
    ...
    echo "thedesk-$(\
        curl -s 'https://thedesk.top/en/' | tr '\n' ' ' | sed 's/\(.*\)thedesk-\(.*\)\.zip"\(.*\)/\2/'
        )-tarball"
}
...
function APP_GET {
    ...
    AGver=$(curl -s 'https://thedesk.top/en/' | tr '\n' ' ' | sed 's/\(.*\)thedesk-\(.*\)\.zip"\(.*\)/\2/')
    AGname="thedesk-$AGver.zip"
    $AGpre curl -O -L "$(curl -s 'https://thedesk.top/en/' | tr '\n' ' ' | \
        sed 's/\(.*\)"\(.*zip\)"\(.*\)/\2/')"
    $AGpre mkdir -p "thedesk-$AGver-tarball"
    $AGpre unzip "$AGname" -d "thedesk-$AGver-tarball"
    $AGpre rm "$AGname"
}
