...
function APP_VERSION {
    ...
    curl -s 'https://retroshare.cc/downloads.html' | tr '\n' ' ' | sed 's/\(.*\)\(retroshare-gui-\(.*\)-x86_64\.AppImage\)\(.*\)/\2/'
}
...
function APP_GET {
    ...
    $AGpre curl -O -L "https://download.opensuse.org/repositories/network:/retroshare/AppImage/$(
        curl -s 'https://retroshare.cc/downloads.html' | tr '\n' ' ' | sed 's/\(.*\)\(retroshare-gui-\(.*\)-x86_64\.AppImage\)\(.*\)/\2/'
    )"
}
