...
function APP_VERSION {
    ...
    echo "$APP_PREFIX$(curl -s 'https://haxe.org/download/' | tr '\n' ' ' | sed 's/\(.*\)<h1>Haxe \(.*\)<\/h1>\(.*\)/\2/')$APP_SUFFIX"
}

# Code to effectly do the download
function APP_GET {
    local AGpre AGver AGdir AGini AGpath AGname
    ...
    AGdir="$(APP_VERSION)"
    AGver="$(echo $AGdir | sed "s/$APP_PREFIX\(.*\)$APP_SUFFIX/\1/")"
    AGname="haxe-$AGver-linux64.tar.gz"
    
    pushd "$AGini" &> /dev/null
    $AGpre curl -O -L "https://github.com/HaxeFoundation/haxe/releases/download/$AGver/$AGname"
    $AGpre tar -zxf "$AGname"
    $AGpre rm "$AGname"
    $AGpre mv haxe_* "$AGpath/$AGdir"
    popd &> /dev/null
